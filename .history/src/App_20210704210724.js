...
import { stringBoardValidation } from "./services/Constants";
import "./App.css";
...
const App = () => {
  ...
  const getValueListner = async () => {
    let inialString = "";
    fetch("http://localhost:3000/easy.txt")
      .then((r) => r.text())
      .then((text) => {
        inialString = text;

        const parseGrid = inialString.replace(/[^0-9\.]/g, "");

        const status = await stringBoardValidation(parseGrid);
        console.log(parseGrid, status);
      });
  };
